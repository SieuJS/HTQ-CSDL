...
app.post('/getPatientInfo', async function(req, res) {
  ...
  try {
    ...
    // Chuyen sang procedure
    const result = await request.execute('proc_ThongTinCaNhanKhachHang')
...
app.post('/updatePatientInfo', async function(req, res){
  ...
  try {
    const request = pool.request();
    request.input('phoneNum', sql.Char,enterPatientPhoneNumber);
    request.input('fullName', sql.Char, patientFullName);
    request.input('Dob', sql.Date, patientDateOfBirth);
    request.input('patientAddress', sql.Char, patientAddress);
    const result = await request.execute('proc_ChinhSuaThongTinKhachHang');
...
app.post('/addPatientInfo', async function(req, res){
  ...
  try {
    const request = pool.request();
    request.input('phoneNum' , sql.Char, patientPhoneNumber);
    request.input('fullName', sql.NChar, patientFullName);
    request.input('password', sql.Char, patientPassword);
    request.input('Dob', sql.Date, patientDateOfBirth);
    request.input('address', sql.NChar, patientAddress);
